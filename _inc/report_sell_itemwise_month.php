...
if (isset($request->get['month'])){
  $month=$request->get['month'];
  $from = date('Y-m-d',mktime(0,0,0,$month,1,date("Y")));
  $to = date('Y-m-d',mktime(0,0,0,$month+1,0,date("Y")));
...
$table = "(
  ...
  WHERE $where_query
  GROUP BY selling_item.item_id) as selling_item";//selling_item.item_id
